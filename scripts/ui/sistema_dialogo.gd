...
@export var printar_caminho_node_funcao: bool = false
...
func _funcao_resource(item: FuncaoDialogo) -> void:
	...
	if printar_caminho_node_funcao:
		print("Caminho do node com a função: ", caminho)
	...
	if item.esperar_sinal_para_continuar:
		...
		if node_com_funcao.has_signal(nome_sinal):
			# Isso aqui é só porque funções lambda
...
func _escolha_resource(item: EscolhaDialogo) -> void:
	...
	for elemento in item.texto_escolha.size():
		...
		if funcao_resource:
			...
			botao_dialogo_variavel.connect(
				"pressed",
				_botao_escolha_pressionado.bind(
					node_com_funcao,
					funcao_resource.esperar_sinal_para_continuar
				),
				CONNECT_ONE_SHOT
			)
			...
			botao_dialogo_variavel.connect(
				"pressed",
				_botao_escolha_pressionado.bind(null, ""),
				CONNECT_ONE_SHOT
			)
...
func _botao_escolha_pressionado(
	...
	# Dá pra pôr o AudioStreamPlayer do pressionamento
	# do botão aqui no sistema de diálogo
	# e tocá-lo aqui.
